...
function Courses() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // console.log(getResponse);
      setEnrolledCourse(getResponse.user.courses);
    }
    catch (e) {
  ...
  //  api call for course enrollments
  ...
  const enrollCourse = async (id) => {
    ...
    // console.log(id)
    try {
      const res = await fetch(`${url}/api/v1/user/enroll/courses/${id}`, {
        method: "POST",
        headers: {
          'token': localStorage.getItem('token'),
          'Content-Type': 'application/json',
      ...
      // setEnrolledCourse([...enrolledCourse, id])
      fetchProfile();
      swal({
        title: "Success",
  ...
  return (
    <>
      ...
      <div className='container'>
        <h2 className='allCourses-heading-desktop'>All Courses</h2>
        <div className='row course-desktop-view '>
          ...
          <div className='col'>
            {
              courses.map((e, id) => {
                const isEnrolled = enrolledCourse.includes(e._id);
                return (
                    ...
                    <div key={e._id} className='course-card-box'>
                      ...
                      <div className='card-row-3'>
                        ...
                        <button onClick={() => { enrollCourse(e._id) }}
                          className={` ${isEnrolled ? 'disabledEnrollNow' : 'enrollNow'}`}>{isEnrolled ? 'Enrolled' : 'Enroll Now'}</button>
                      </div>
                    </div>
